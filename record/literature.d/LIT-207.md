...
- version: 2
  date: '2026-09-25'
  note: >-
    Separates this paper's contribution from one it inherited. The summary said
    "A decoder-only transformer without positional encoding is shown to
    represent both absolute and relative position anyway" — the absolute half
    of that is Haviv et al. 2022 (LIT-tmpbi5gf), a year earlier, with the
    probing and the causal-mask mechanism. What is this paper's is the
    length-generalization comparison: five schemes under identical
    hyperparameters, and the one that extrapolates is none at all. The relative
    half is also its own. SOTA-153 had named this paper as the source of the
    implicit-position result; that is corrected in the same contribution.
tags:
...
# Secondary, restored: length generalization is an attention-techniques question as much as an encoding one (ADR-035).
...
summary: >-
  ...
  lengths is none at all. It also reports that such a model represents both
  absolute and relative position anyway, which is why removing the encoding
  removes a constraint rather than information — though the **absolute** half
  of that is Haviv et al. 2022 (LIT-tmpbi5gf), a year earlier and with the
  mechanism.
extended_by:
- LIT-208
- LIT-209
