...
# LIT-160: DeepSeek-V3 Technical Report
...
- The model everything in the record's DeepSeek line descends from: 671B
  total with 37B activated, MLA ([LIT-174](LIT-174.md)) and DeepSeekMoE
  ([LIT-170](LIT-170.md)) carried over from V2, pretrained on 14.8T tokens.
- Two things it introduces. An **auxiliary-loss-free load balancing
  strategy** ([LIT-171](LIT-171.md)), and a **multi-token-prediction training
  objective** ([LIT-163](LIT-163.md)) — both of which the subsequent frontier models in
  the record ship.
...
line the record now holds end to end: V2's MLA ([LIT-174](LIT-174.md)) → this →
