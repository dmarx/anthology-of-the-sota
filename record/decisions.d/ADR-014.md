...
- version: 1
  date: '2026-09-06'
  note: >-
    Filed with the phrasing rule "name a kind of result, not a count", which
    read as though only evidential kinds qualified.
- version: 2
  date: '2026-09-06'
  note: >-
    Amended, not superseded: a threshold of adoption is a kind of result too.
    The rule was narrower than the mechanism — two of the eight conditions
    written under it were already social — and resolution in a field is
    largely a social process (ADR-015). The requirement is unchanged.
tags:
- record
- mechanism
date: '2026-09-05'
...
**A threshold of adoption is a kind of result.** The rule above is about
evidence being the *right* evidence, not about it being experimental. Whether
a field has taken something up is a fact about the world, it is often the
thing a practice is actually waiting on, and resolution in a research
community is largely social rather than decisive ([ADR-015](ADR-015.md)). So
*"a frontier training report that adopts it and says so"* is a well-formed
condition, and [SOTA-120](../practices.d/SOTA-120.md) — where the paper was settled and only the adoption
was in question — could not have been written any other way. What makes such
a condition good is the same thing as before: it names what would count, so
the wrong kind of report cannot satisfy it.

That one has since been **collected**, which is the outcome this ADR was
betting on. `LIT-156` benchmarks ten optimizers against a well-tuned AdamW
and `LIT-122` adds decoupled decay to Muon as a stated design decision;
neither was written to settle `SOTA-120`, and both do. A social condition is
not a weaker condition — it is one whose evidence arrives as a side effect of
other people's work, and it is met when the record catches up rather than
