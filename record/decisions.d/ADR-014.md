...
<!-- inactive-ok: SOTA-120 — a Deferred practice, named as the case only a social condition fits -->
condition, and [SOTA-120](../practices.d/SOTA-120.md) — where the paper is settled and only the adoption is
in question — could not have been written any other way. What makes such a
condition good is the same thing as before: it names what would count, so the
wrong kind of report cannot satisfy it.
