...
- **CPC / InfoNCE** — `LIT-tmp5vq8g`, `SOTA-tmp6lc4c` and `THEORY-tmpptbj9`.
  ...
  **`THEORY-tmpptbj9` files the part the folklore drops.** The paper derives
...
- **`SOTA-359` v2 — CLIP's caption practice now declares `extends:
  SOTA-tmp6lc4c`**, which is what `LIT-588` itself says: CLIP cites Oord et
