...
- `SOTA-tmphckqj` — **do not transfer a data mixing ratio across model
  ...
  follows a power law in model size. Sources `LIT-tmphrxdn` (Gu et al.,
  `ARXIV-2505.18091`), read in `NOTE-tmpp6ac8`.
- `SOTA-tmppfvpd` (`Active`) — keep a knowledge base outside the weights.
  All of Wikidata would want ~1000B non-embedding parameters at 100 epochs,
  and derivable facts cost full price. Sources `LIT-tmpvt6e3` (Lu et al.,
  `ARXIV-2406.15720`, read in `NOTE-tmpk9vmg`) and `LIT-060`.
- `SOTA-tmp8nitg` — start the training distribution imbalanced and flatten
  ...
  `LIT-tmpbtdlg` (Zucchet et al., `ARXIV-2503.21676`), read in `NOTE-tmp7om7u`.
- `THEORY-tmpijbl9` — a model of bounded capacity allocates it across
  datasets like a knapsack, so the optimum jumps rather than sliding.
  Explains `SOTA-tmphckqj`.
- `THEORY-tmp8a4gm` (`Active`) — **the plateau before factual recall is the
  ...
  the plateau. Explains `SOTA-tmp8nitg`.

### Changed

- **`SOTA-166` v2 and `SOTA-238` v3 are both `contested`**, by
  `LIT-tmphrxdn`. Both set data proportions from small runs — one by fitting
...
- `THEORY-025` v2 — corroborated and bounded in the same contribution.
  `LIT-tmpvt6e3` measures fact capacity on Wikidata and finds it linear in
  model size, which is this account's linear claim through a different
  instrument and different units. `LIT-tmphrxdn` bounds it: the
