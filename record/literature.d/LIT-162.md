...
- version: 2
  date: '2026-09-18'
  note: >-
    Adds `attention-techniques`. The paper's central claim is that SSMs and variants of attention are the same object seen from two ends; filing it as architecture alone recorded half of that.
tags:
- model-architecture
...
arxiv: '2405.21060'
# Mamba-2 comes out of the SSM/attention duality applied to Mamba.
extends:
- LIT-161
first_author: 'Dao'
keywords:
...
# LIT-162: Transformers are SSMs: Generalized Models and Efficient Algorithms Through Structured State Space Duality
...
([LIT-161](LIT-161.md)) → the duality and Mamba-2 here → gated delta rule ([LIT-137](LIT-137.md)) →
channel-wise gating in KDA ([LIT-133](LIT-133.md)), with [LIT-165](LIT-165.md) as the 2026
continuation and [LIT-173](LIT-173.md) as the other family's answer.

**No practice of its own, and the reason is what kind of result it is.** The
duality is not an instruction; it is the proof that makes one possible. A
hybrid is a coherent thing to build because the two mechanisms are
commensurable, and this is what establishes that. It is a premise of the
practices rather than a rival to them, which is why it appears in their
reasoning and not in their `source:` lists.
