...
- `SOTA-tmp4dbv9` — train as masked diffusion when the corpus is fixed and
  the compute is not. `extends` `SOTA-157`, adding the condition that one
  lacked. Sources `LIT-tmp5xe2h` (Prabhudesai et al., `ARXIV-2507.15857`),
  read in `NOTE-tmpd2fpc`.
- `SOTA-tmp9rfqy` — tune weight decay upward over a repeated corpus rather
  than inheriting 0.1. `SOTA-tmpdngkz` — ensemble independently seeded models
  and distil, rather than scaling one. `SOTA-tmpa2zin` — judge a monotone
  scaling recipe by its asymptote. All three from `LIT-tmp2udh1` (Kim et al.,
  `ARXIV-2509.14786`), read in `NOTE-tmpc6v83`.
- `THEORY-tmpqd9wu` — a transformer holds about 3.6 bits per parameter, and
  generalization begins where the data outgrows that budget. Sources
  `LIT-tmp28gse` (Morris et al., `ARXIV-2505.24832`), read in `NOTE-tmpbxl0o`.
...
- **`explains:` left empty on a theory, deliberately.** `THEORY-tmpqd9wu`
