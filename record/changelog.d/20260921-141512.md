...
- **The Coverage Principle** (`LIT-tmpkyvap` / `NOTE-tmpxy07e`), Chen, Huang,
...
- **`THEORY-tmp16mrv`** — cross-entropy pays an unbounded price for mass a
  learner never had, and that price grows with sequence length. `Proposed`.
- **`SOTA-tmpkxz0v`** — choose the checkpoint you post-train from by coverage,
