...
- version: 2
  date: '2026-09-24'
  note: >-
    Corrected against a full reading (NOTE-333). "Blind" is dropped from the
    human preference claim, the gradient-variance sentence no longer mentions
    loss spikes, the ablation hedge now names both FID-reporting arms, and
    only the human-preference opponents are called anonymized.
tags:
- generative-modeling
- vision-and-graphics
...
# LIT-619: Wan: Open and Advanced Large-Scale Video Generative Models
...
- **It discloses a recipe.** Weights, code, data pipeline, VAE, training
  ...
  that its authors' own benchmark and human preference put ahead of the
...
- **The curriculum.** It starts with 256px text-to-image pre-training, then
  ...
  small batches, and the report attributes training instability to the
  resulting spikes in gradient variance.
...
- **Where the hedges are.** Per [DP-010](../../docs/design-principles.md#dp-10):
  ...
  - All three ablations run on the 1.3B model, on text-to-image, and two of
    them are judged mainly by training loss. The VAE ablation (Table 5) and
    the Qwen-VL arm of the text-encoder ablation (Table 6) report FID. On
    Table 6, Qwen-VL-7B's second-last layer scores 42.91 against umT5's 43.01. umT5
  ...
  - The human-preference comparisons use anonymized opponents. The named
    comparisons are Wan-Bench, the authors' own, and VBench, a public
    leaderboard. On image-to-video matching, Wan loses to
    CN-TopA (−4.2%, Table 7).
...
- Pre-train a video diffusion model on images at low resolution, then raise
  ...
  and it has no ablation. The image-first half is now [SOTA-386](../practices.d/SOTA-386.md),
...
Read in full as [NOTE-333](../notes.d/NOTE-333.md).
