...
# LIT-433: Learning to summarize from human feedback
...
- **The loop, on text.** Collect human comparisons between candidate
  ...
  `LIT-434`'s construction with a language model as the policy
...
Filed with `LIT-434` because a two-step lineage with the middle missing
