...
# LIT-186: Scaling Laws for Precision
...
<!-- inactive-ok-block: SOTA-tmpa81q9 — the practice drawn from this paper,
     Proposed, and saying why is what this paragraph does -->
**The practice is [SOTA-tmpa81q9](../practices.d/SOTA-tmpa81q9.md), `Proposed`.** The inference result is what it
recommends: if a model will be served quantized, the token budget and the
quantization plan are one decision. It is `Proposed` because the fits stop at
1.7B and 26B tokens, far below everything that ships FP4 — the effect is well
measured where it was measured, and the regime it matters most in is not that
regime.
