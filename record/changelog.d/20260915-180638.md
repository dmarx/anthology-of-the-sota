...
- **`SOTA-154` is `Active`** — fine-tune with evolution strategies instead of
  ...
  Countdown and the conciseness objective", and `LIT-tmphm6g2` is that: six
...
- **`SOTA-154`'s consensus moved from `unreplicated` to `contested`**, with
  `contested_by: LIT-tmp9pcfv`. Both halves of that are new in the same edit
...
- **`SOTA-tmpm80i3` is `introduced_by: LIT-211`, not by its own source.** The
...
- **`LIT-tmp4zb0l` — Evolution Strategies at the Hyperscale** (Sarkar et al.,
...
- **`LIT-tmp9pcfv` — Hyper-ES** (Gu et al., 2026). `corrects:` `LIT-211`, by
...
- **The record now offers a reading of its own dispute.** Hyper-ES tests only
  models at 1.5B and below, and `THEORY-tmp38myz` holds that the density of
  ...
  record's and is labelled as such, in both `SOTA-154` and `LIT-tmp9pcfv`.
