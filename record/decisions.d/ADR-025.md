...
- version: 2
  date: '2026-09-09'
  note: >-
    Amended: the absence of a NOTE means the paper is unread, so no
    Unread stubs are filed. Unread stays in the vocabulary for a
    deliberate human judgement — looked at and set aside — which absence
    cannot express. Decided after luria's inert-status check fired on
    this scheme at 12/12 Read.
tags:
- ontology
- record
date: '2026-09-09'
...
- **`Unread`** — looked at and deliberately set aside. A human judgement, not
  a backlog entry; see *Absence means unread* below.
...
Amended at version 2, after luria's inert-status check fired on this scheme
at **12 of 12 `Read`** — correctly, since a note is only written when someone
finishes a paper.

**A `LIT` with no `NOTE` means nobody has read that paper.** That is exactly
the state the record could not previously express, it costs nothing to
express it this way, and it makes the check's finding the honest shape of the
field rather than a defect. The alternative — filing two hundred `Unread`
stubs so the distribution looks healthy — is a migration that buys a row in a
table and makes every future reader wonder whether a stub means "unread" or
"read and found empty".

`Unread` stays in the vocabulary for the case absence cannot cover: **a
deliberate human judgement that a paper was looked at and set aside**.
"Nobody has got to this yet" is silence. "I opened this and decided not to
read it" is a fact, and it needs somewhere to live.

So the inert-status warning will keep firing while every filed note is a
finished reading, and that is correct. It stops being predictable the first
time someone uses `Unread` for what it is now for.
