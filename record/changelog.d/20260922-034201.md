...
- **`LIT-tmp4uno1`** — *Grokking: Generalization Beyond Overfitting on Small
  Algorithmic Datasets* (Power, Burda, Edwards, Babuschkin and Misra, 2022),
  read as `NOTE-tmphxkj0`.
- **`LIT-tmpkn1i6`** — *Omnigrok: Grokking Beyond Algorithmic Data* (Liu,
  Michaud and Tegmark, 2022; ICLR 2023), read as `NOTE-tmpi8cjd`.
- **`LIT-tmp9xrey`** — *Explaining grokking through circuit efficiency*
  (Varma, Shah, Kenton, Kramár and Kumar, 2023), read as `NOTE-tmpp2k64`.
- **`LIT-tmp069e7`** — *Grokking as the Transition from Lazy to Rich Training
  Dynamics* (Kumar, Bordelon, Gershman and Pehlevan, 2023; ICLR 2024), read as
  `NOTE-tmpvrr0d`.
- **`THEORY-tmpdlyut`** — grokking is a regime rather than a property of
  algorithmic data, and at least three knobs move it. `Active`.
- **`THEORY-tmpz9vjh`** — the LU mechanism: training and test loss disagree as
  functions of weight norm. `Proposed`.
- **`THEORY-tmpmiiyl`** — circuit efficiency and the critical dataset size.
  `Proposed`.
- **`THEORY-tmplnntp`** — grokking is the transition from lazy to rich
...
- **The founding paper reported the regime dependence itself.** `LIT-tmp4uno1`
...
- **Data fraction is one axis of three.** `LIT-tmpkn1i6` induces grokking on
  **MNIST** (1k examples, Kaiming weights scaled by `α > 1`), **IMDb** with an
  ...
  algorithmic data by constraining the weight norm. `LIT-tmp069e7` adds initial
  neural-tangent-kernel alignment, measurable on any task as centered kernel
  alignment. That is `THEORY-tmpdlyut`, and it is `Active` because it is the
  one claim all four papers support.
- **Three mechanisms, none of them settled, filed separately.** The LU
  mismatch (`THEORY-tmpz9vjh`), circuit efficiency (`THEORY-tmpmiiyl`) and
  lazy-to-rich (`THEORY-tmplnntp`). All `Proposed`. `Active` on this scheme
...
- **The counterexample is why the unit is four papers rather than two.**
  `LIT-tmp069e7` §3: modular arithmetic, two-layer MLP, **no weight decay**,
  ...
  `THEORY-tmplnntp` declares `corrects` on both.
- **And the correction does not finish the job, which the record says rather
  than smoothing over.** `THEORY-tmpmiiyl` derived **ungrokking** — a grokked
  ...
  **semi-grokking**, and then observed both. Nothing in the lazy-to-rich
  ...
  `THEORY-tmplnntp`'s `promote_when`.
- **The record's existing grokking paper turns out to be an antecedent.**
  `LIT-tmp9xrey` names `LIT-085`'s Appendix E as the explanation genre it makes
