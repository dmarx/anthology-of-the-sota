...
- version: 2
  date: '2026-09-26'
  note: >-
    Its two evaluation metrics now have a definition in the record.
    LIT-tmp0ve4d supplies infidelity and max-sensitivity, and with them the
    property this note reported around without: max-sensitivity is minimised by
    a constant explanation, so "max-sensitivity remains almost unchanged" is
    weaker evidence of trustworthiness than it reads as. The infidelity figures
    are unaffected and this note already named its perturbations, which is what
    makes them comparable to themselves.
tags:
...
- **The two metrics it scores with are now held, and one of them needs a
  caveat this note reported without.** [LIT-tmp0ve4d](LIT-tmp0ve4d.md) defines both.
  Max-sensitivity is minimised by **a constant explanation**, so "max-sensitivity
  remains almost unchanged" under randomization is a weaker signal than it
  reads as — a measure an input-ignoring attribution wins cannot carry
  trustworthiness alone. Infidelity is defined relative to a perturbation
  distribution, which this note does specify (`N(0, 0.03)`, and `x − x₀` for
  global IG); that is what makes its figures comparable to each other and not
  to anyone else's. [SOTA-tmp34edx](../practices.d/SOTA-tmp34edx.md) is the practice.
