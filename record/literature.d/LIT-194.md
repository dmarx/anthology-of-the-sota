...
**No practice of its own: parallel invention, not a step the record's line
took.** Data-dependent per-channel gating arrived here a year before the
paper that this record's recommended module actually builds on, and by a
different construction — which is worth holding as a second data point and is
not a recommendation the record can source to anything it recommends. Its
other contribution, the kernel library both parents ship in, is
infrastructure the practices assume rather than advice they give.
