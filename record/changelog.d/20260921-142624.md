...
- **Dropout Reduces Underfitting** (`LIT-tmpfdpkj` / `NOTE-tmpbrbwv`), Liu,
...
- **`SOTA-tmpxll55`** — put dropout at the start if the model underfits and at
  the end if it overfits; the schedule decides the sign, not the rate.
  `Proposed`, `unreplicated`.
- **`THEORY-tmpbaqnm`** — early dropout buys a *biased* gradient estimate with
...
- **What is replicated and what is one model.** The variance and error
  ...
  **crossing point** — the thing that would tell you where to switch — is
  ViT-T on ImageNet only. `THEORY-tmpbaqnm` is `Proposed` for exactly that
