...
- **`LIT-tmpitbks`** — Parallel Scaling Law for Language Models (Chen et al.,
  2025), read as `NOTE-tmp5l9o0`. Run the same weights over `P`
...
- **`SOTA-tmp9spic`** — scale parallel computation with learnable input
...
- **`SOTA-tmp91kxm`** — add the parallel streams in a short final training
...
- **`THEORY-tmprktkw`** — parameters carry memorization and parallel
...
- **The cost analysis is denominated in memory and latency rather than
  ...
  `SOTA-tmp9spic` rather than as its own practice: the argument is good and
...
- **`THEORY-tmprktkw` is `Proposed` for a reason worth stating.** Two
