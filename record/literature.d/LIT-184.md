...
# LIT-184: The FineWeb Datasets: Decanting the Web for the Finest Text Data at Scale
...
[LIT-178](LIT-178.md), which argues this filtering is too aggressive for long token
horizons.

**No practice of its own, on scope rather than adoption.** A dataset is not a
technique, and the record should not recommend a corpus. The transferable
content — that filtering and deduplication choices are worth measuring
individually rather than adopted as a bundle — is methodology, and the
practice it bears on is the filtering-horizon one drawn from [LIT-178](LIT-178.md), which
names this as the position it argues against. Neither contradicts the other:
this measured filters at the horizon it measured them at, and that one says
the answer moves with the horizon.
