...
- **The record's first statement about how a visual encoder gets pretrained**
  (`#85`, `#154`). `SOTA-tmpkip7h` — predict the representations of masked
...
- **`SOTA-tmpzrzg8`, `Proposed`** — train at low resolution and raise it only
...
- **Three readings**: `NOTE-tmp91b9u` (`LIT-216`), `NOTE-tmp9kngh`
  (`LIT-215`), `NOTE-tmpygxgb` (`LIT-218`).
...
- **One lab, twice, is not two groups agreeing.** `LIT-215` scales `LIT-216`'s
  ...
  `SOTA-tmpkip7h`'s `consensus_note` rather than its `source:` — `ADR-017`,
