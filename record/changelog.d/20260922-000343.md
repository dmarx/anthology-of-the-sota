...
- **`LIT-tmpmftzj`** — *Small Singular Values Matter: A Random Matrix Analysis
  of Transformer Models* (Staats, Thamm and Rosenow, 2024), read as
  `NOTE-tmpsv1p1`.
- **`LIT-tmphxv7m`** — *From Low Rank Gradient Subspace Stabilization to
  Low-Rank Weights* (Jaiswal et al., 2024), read as `NOTE-tmpzvhd1`.
- **`LIT-tmpx9iti`** — *Complexity-Guided Component-wise Initialization for
  Language Model Pretraining* (Garbers and Oh, 2026), read as
  `NOTE-tmpptsb7`.
- **`LIT-tmpxqt7b`** — *The Stability of Singular Distribution* (Zhang et al.,
  2026), read as `NOTE-tmpg1l16`.
- **`LIT-tmptsr98`** — *The Truth is in There* (Sharma, Ash and Misra, 2023),
  filed on a partial reading for the debate it anchors.
- **`SOTA-tmpsbgvf`** — choose the rank per matrix when compressing a
  transformer. `Active`, `unreplicated`.
- **`SOTA-tmp6xtka`** — do not rank singular directions by magnitude when
...
- **It reconciles two published results that contradicted each other.** One
  ...
  improved reasoning (GPT-J on CounterFact, 13.3% → 24.1%, `LIT-tmptsr98`).
