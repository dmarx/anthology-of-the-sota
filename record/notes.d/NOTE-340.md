...
- version: 2
  date: '2026-09-25'
  note: >-
    ADM is now held, LIT-699, read in full. Its Table 11 confirms the
...
- **ImageNet-128:** FID 20.9 and NLL 2.90, compared only against GANs
  ...
  ([LIT-699](../literature.d/LIT-699.md), Table 11: 422M parameters) is **class-conditional**; this
  model is not.
...
| C7 | FM converges faster in training | weak | Fig. 5 curves on ImageNet-64 with no values in text. The ImageNet-128 throughput comparison is against ADM's published run, not a controlled one, and that run is class-conditional while FM's is not ([LIT-699](../literature.d/LIT-699.md), Table 11) |
