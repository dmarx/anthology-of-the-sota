...
- **All 50 remaining generic-bullet `LIT` documents read**, with takeaways
  replaced by the papers' own mechanisms, numbers and scope. 46 `Read`, 4
  `Skimmed`. Every reading files a `NOTE`, so the record no longer has documents
  that look read and are not.
