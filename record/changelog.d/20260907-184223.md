...
- Three practices in the schedule and µP cluster now name every paper they
  rest on. Second batch of #58: six examined, three changed.
  - `SOTA-140` names `LIT-145`. Its Source section had said "Hägele et al.
    (2024), LIT-145, for the comparison against cosine" since it was filed;
  ...
  - `SOTA-141` names `LIT-145`. Its `consensus_note` already read "with
    LIT-145's cooldown pointing the same way", so the consensus value rested
    on a paper the source list did not name — exactly the disconnect ADR-010
...
- No recommendation changed; `history:` entries per ADR-019.
