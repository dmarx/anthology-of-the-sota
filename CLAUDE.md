...
qualify if the recommendation is one the eighteen topics can express
...
- **The first tag is the primary topic, and a document may carry more than
  one.** The eighteen live in the `topics` vocabulary in `luria.yaml` — one
...
Or `make ready`, which runs those three and discards the views ([ADR-054](record/decisions.d/ADR-054.md)).
