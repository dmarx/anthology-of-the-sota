...
- version: 2
  date: '2026-09-18'
  # inactive-ok-block: ADR-036 — Superseded, and named here as the decision
  # that reached the opposite verdict; the citation is to what it decided,
  # not to a rule still in force
  note: >-
    Adds `model-architecture`. The title names two contributions —
    conditional computation and automatic sharding — and only the second was
    tagged. ADR-036 identified this as a trunk paper spanning two topics and
    declined to split it; under ADR-046 it carries both instead.
tags:
