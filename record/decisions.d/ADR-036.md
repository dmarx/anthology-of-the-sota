...
number: 36
status: Superseded
# inactive-ok: ADR-048 — Superseded in its turn; the chain is deliberate
superseded_by: ADR-048
status_note: >-
  Superseded only in its fourth category. The four-way classification, the
  fifteen retags and the two removed sibling links all stand; what changed is
  the verdict on trunk papers whose contribution spans two topics, which this
  decision left standing because splitting one would file it under half its
  contribution. [ADR-046](ADR-046.md) made splitting unnecessary — such a paper carries both
  topics — so the successor retags six documents rather than leaving them.
formerly:
- ADR-tmpzwkwt
...
# ADR-036: An unbound relation is one of four things, and only one of them is a retag
