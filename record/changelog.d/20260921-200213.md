...
- **`LIT-tmpm2gzh`** — *Opening the Black Box of Deep Neural Networks via
  Information* (Shwartz-Ziv and Tishby, 2017), the claim.
- **`LIT-tmpsld21`** — *On the Information Bottleneck Theory of Deep Learning*
  (Saxe et al., 2018), the rebuttal, read as `NOTE-tmp11zjy`.
- **`LIT-tmpcnvbw`** — *Adaptive Estimators Show Information Compression in
  Deep Neural Networks* (Chelombiev, Houghton and O'Donnell, 2019), read as
  `NOTE-tmplxznx`.
- **`SOTA-tmpgvchd`** — state the noise or binning assumption behind any
  mutual information you report for a deterministic network, and show the
  conclusion survives changing it. `Active`, `emerging`.
- **`THEORY-tmppkfku`** — apparent compression in the information plane is
...
- **The sensitivity is demonstrated twice, in opposite directions, by papers
  arguing against each other.** `LIT-tmpsld21` re-bins the same `tanh` run
  ...
  `log₂(P)`. `LIT-tmpcnvbw` re-bins ReLU adaptively per layer and epoch and
...
- **The three IB claims do not have the same status, and saying "the dispute
  ...
  refuted by `LIT-tmpsld21`'s four-cell dissociation — compress-and-generalize,
  compress-and-overfit, neither, both — and independently by `LIT-tmpcnvbw`,
...
- **`LIT-tmpcnvbw`'s abstract is not supported by its own protocol.** It says
