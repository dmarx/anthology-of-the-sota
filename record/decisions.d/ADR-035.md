...
- version: 2
  date: '2026-09-21'
  note: >-
    Adds a section stating the rule positively. Nothing in the decision
    changes. Raised in review: CLAUDE.md compressed this decision to "add a
    second topic when it is genuinely true, and never to bind a relation",
    and the second clause was then read as forbidding the very workflow the
    unbound-lineage report exists to support — a real topic added after the
    report pointed at the edge. That is not what this decision says; §3 calls
    a shared tag binding a relation "the point and the risk", and the risk it
    names is a free-form label, which the closed vocabulary has since made
    structurally unavailable on SOTA, LIT and THEORY. The gloss was the
    defect and CLAUDE.md is corrected in the same contribution; this section
    exists so the next compression has something unambiguous to compress.
- version: 3
  date: '2026-09-21'
  note: >-
    `Proposed` to `Active`. The condition this decision set for itself was not
    a result but a person: "filed `Proposed` because that reading should be
    checked by someone who was there" — the reading being that `exactly-one` was
    load-bearing for the vocabulary being shared and small, and never for a
    document having only one subject. The repository owner, who made both
    earlier calls, reviewed it while correcting CLAUDE.md's gloss of it and
    said to set it Active. The condition is met, by the only thing that could
    have met it.
tags:
- taxonomy
- record
...
cardinality. This decision was filed `Proposed` because that reading needed
checking by someone who was there; it was checked and the decision is `Active`
as of v3.
...
## What became of this decision's warning

*(Added at v2, after a compression of this decision in `CLAUDE.md` was read as
saying the opposite of what it says.)*

The warning in §3 and §4 — that a shared tag binding a relation is "the point
and the risk", the risk being a *free-form label* invented to satisfy the
check — has been narrowed twice since. A reader arriving here should go to
those decisions rather than reconstruct the rule from this one.

- **[ADR-046](ADR-046.md)** — tag the document's subject, liberally; the test is whether a
  tag is *justifiably appropriate*, and the forbidden case is narrow. It says
  this decision "stands unchanged" and that a tag true of a document "is not
  made suspect by a chain that might one day read it".
- **[ADR-049](ADR-049.md)** — an unbound relation has exactly two readings, the invariant is
  missing or the relation is wrong, "and never a third where the answer is to
  note that somebody looked". It took 22 unbound relations and 10 unbound
  lines to zero by naming what twenty-one documents were already about, and
  rejected an acknowledgement directive for the purpose.

Together those say what this decision left implicit: the report exists to find
documents that are not saying what they are about, and answering one by adding
a true topic is the intended response. What stays forbidden is narrow —
inventing a label to satisfy the check, which §5's closed vocabulary has since
made structurally unavailable on `SOTA`, `LIT` and `THEORY`.
