...
- **Estimating the Probability of Sampling a Trained Neural Network at
  Random** (`LIT-tmpk8scf` / `NOTE-tmpw2aa7`), Scherlis and Belrose (2025).
...
- **`THEORY-tmp8wi2a`** — how much of parameter space behaves like your
  trained network is a description length, and the better-generalizing
  network occupies more of it. `Proposed`.
- **`SOTA-tmpczmhy`** — to find a model that generalizes badly where its
  outputs look fine, measure that region. `Proposed`, `unreplicated`.

- **`DP-tmpymzzl`** — the most citable sentence in a paper is often its least
