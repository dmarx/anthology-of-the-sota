...
- version: 2
  date: '2026-09-25'
  note: >-
    Standing no longer implies every adopter fine-tunes the MTP head into a
    draft model: DeepSeek-V4's full report (NOTE-tmpr3mtt) keeps it only as a
    training objective.
tags:
...
Kimi K3's head is fine-tuned into. Not every adopter uses the head that way on
its own evidence: DeepSeek-V4's full report ([NOTE-tmpr3mtt](../notes.d/NOTE-tmpr3mtt.md)) keeps it purely as
a training objective and never mentions speculative decoding.
