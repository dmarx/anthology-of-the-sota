...
- version: 2
  date: '2026-09-24'
  note: >-
    Corrected against a full reading (NOTE-tmpeb8mt). Reconstruction FID now
    gets worse only on driving and better on WebVid and mountain biking; the
    trained-parameter count is attributed to the paper and noted to omit the
    fine-tuned spatial layers and decoder; the image-pretraining ablation now
    says its images are frames of the same videos and its steps unmatched.
tags:
- generative-modeling
- vision-and-graphics
...
summary: >-
  ...
  (534 against 1155). The paper counts 2.2B of the text-to-video model's 3.1B
  parameters as trained, and that count leaves out the spatial layers and
  decoder it also fine-tuned. The abstract describes training only temporal
  layers.
extended_by:
- LIT-625
compared_against:
...
- **Image pretraining is the controlled result.** On driving scenes, against
  ...
  layers, matched on trainable parameters, reach 704. The pretraining images
  are 1 fps frames of the same driving videos, and the end-to-end arm gets no
  equivalent of the 73K image-pretraining steps (App. H.1, Table 7).
- **The autoencoder has to learn video too.** Fine-tuning the decoder on
  ...
  (Table 11). Reconstruction FID gets slightly worse on driving (7.61 →
  9.17) and slightly better on WebVid (13.89 → 11.68) and mountain biking
  (20.76 → 18.65). Adding an image
...
- **"Only need to train a temporal alignment model"** (abstract) is not what
  ...
  was fine-tuned. The paper counts 2.2B of 3.1B as trained (App. H.2.1): the
  656M temporal layers and the 1,509M interpolation model. That count leaves
  out the 865M spatial layers, fine-tuned on WebVid frames first, and the
  fine-tuned decoder. Only the encoder and text encoder were never updated.
...
Read in full as [NOTE-tmpeb8mt](../notes.d/NOTE-tmpeb8mt.md).
