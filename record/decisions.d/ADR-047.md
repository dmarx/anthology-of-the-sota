...
summary: >-
  ...
  Writing `extends`'s blurb answered the open question from [ADR-048](ADR-048.md): it
...
<!-- inactive-ok-file: ADR-048 — Superseded by ADR-tmphn2vp after this
     decision was written; every mention here is of what it asked for
     at the time, which is the history being recorded -->

# ADR-047: Every relation says what it means, and extends already covered specialization
...
The cost was not theoretical. `ADR-048` closed asking for a
...
- **Add `specializes:`.** What `ADR-036` and `ADR-048` both asked for.
...
**`ADR-048`'s standing request is withdrawn**, and the `SOTA-089` ↔
