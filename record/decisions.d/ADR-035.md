...
*(Added at v2, after the compression of this decision in `CLAUDE.md` was read
as saying the opposite.)*

**The test for a second topic is whether it is true of the document. That is
the only test.** Not whether it binds an edge, and not whether you noticed it
because something pointed at the edge.

`docs/reports/unbound-lineage.md` exists to find relations the record asserts
without saying what they have in common. Most such findings have one of three
causes: the relation is wrong, the vocabulary is short a word, or **a document
is not saying what it is about**. The third is the common one, and the fix is
to add the true topic. Acting on the report that way is the intended response,
not a way around a rule — §3 above does exactly this, weighing three
newly-bound edges and keeping two.

What this decision rules out is narrower and worth naming exactly: **inventing
a label to satisfy the check.** That is the [#101](https://github.com/dmarx/anthology-of-the-sota/issues/101) move, where
`flash-attention` was added to both ends of an edge that should have stayed
open, and §4 removes it. The failure is not "a tag that binds"; it is "a tag
that is not a topic, added because it binds".

On `SOTA`, `LIT` and `THEORY` that failure is now **structurally unavailable**.
§5 closed the vocabulary on those three schemes, so the only tag that can be
added is one of the thirteen — each with a label, a blurb and a decision
behind it — and the truth test is the only question that remains. `NOTE` is
still the hole, as the consequences section says.

**Where truth and binding come apart, truth wins and the edge stays open.** If
the honest tag set does not bind a declared relation, the report should keep
reporting it: either the relation is wrong and should be dropped, or the
vocabulary is short a word and the intended move is to add one. Closing a
report by asserting something false is the only move this decision forbids.
