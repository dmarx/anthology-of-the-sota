...
- version: 3
  date: '2026-09-18'
  note: >-
    The lockfile's size is no longer quoted as a number. It said "305 entries"; the count was wrong when written — measured on a branch carrying two entries main did not have — and is now stale besides. What the paragraph needs is that the file is ONE file holding one entry per cited identifier, which is true at any size.
tags:
- record
- mechanism
date: '2026-09-08'
...
derived — and was still a file every paper-filing contribution rewrote: one
file, one entry per cited identifier, touched by fifteen consecutive
contributions. The writer was the linter,
