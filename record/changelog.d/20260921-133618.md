...
- **The emergence dispute, both sides** (`LIT-tmpc4fk2` / `NOTE-tmpna4sp`,
  `LIT-tmpgnhq2` / `NOTE-tmpnb7kf`). Wei et al. (2022), which named and
  ...
  disagreement.
- **`THEORY-tmpacv6c`** — a metric that composes or thresholds per-token
...
- **The popular framing of this dispute is wrong, and reading the primary
  ...
  is not what happened. `LIT-tmpc4fk2` §5.1 raises the metric explanation
...
- **What `LIT-tmpgnhq2` does not claim, in its own words**: "nothing in this
