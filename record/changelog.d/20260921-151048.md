...
- **The Diffusion Duality** (`LIT-tmpvrm26` / `NOTE-tmp290m1`), Sahoo,
...
- **`THEORY-tmprosv3`** — the duality, and why it exists for uniform-state
  diffusion and **not** for masked diffusion. `Active`: both halves are
  derived and one is checked numerically.
- **`SOTA-tmpzmile`** — when the sampling budget is small, prefer uniform-state
