...
- version: 2
  date: '2026-09-18'
  note: >-
    Two tags added. Version 1 carried `data-pipeline` alone on the reasoning
    that the record holds this paper for C4 and an incidental tag binds
    chain edges. That was too cautious on both counts: the text-to-text
    framework and the systematic transfer-learning ablation are the paper,
    not incidental to it, and this note declares no relation for an
    invariant to bind. The record's reason for holding a document and the
    document's subject are different questions.
tags:
- data-pipeline
- model-architecture
...
# LIT-425: Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer
