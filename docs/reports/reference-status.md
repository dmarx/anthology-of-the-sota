...
**5 documents cited without acknowledgement.** Not listed: 1876 citations someone has already vouched for with an `inactive-ok:` comment at the citing site.
...
**0 codes not minted here.** Not listed: 4 mentions marked deliberate with an `unresolved-ok:` comment — an illustrative code in an example is the usual one.
