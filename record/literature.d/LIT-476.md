...
`d/2` is a strict upper bound not seen in practice ([LIT-tmp6dook](LIT-tmp6dook.md); `d/2` is the
regular-model case of the volume law, and `λ ≤ d/2` is the content of the
singular one — see [THEORY-tmpzuan6](../theory.d/THEORY-tmpzuan6.md)). The authors attribute the
