...
<!-- inactive-ok-file: SOTA-411 SOTA-403 THEORY-103 — all Proposed; the practice this
     paper sources, a practice whose consensus note it amends, and an account
     it adds one uncontrolled data point to -->

# LIT-683: Scaling up Test-Time Compute with Latent Reasoning: A Recurrent Depth Approach
...
sources one `Proposed` practice, `SOTA-411`, with a `promote_when` that
asks for the missing FLOP-matched baseline.

**A same-author data point pulls the other way.** [LIT-681](LIT-681.md)
