...
- `LIT-tmpfv7vs` / `NOTE-tmpepjgm` — Zheng et al., *Spectral Condition for
...
- `THEORY-tmpmnsb5` — residual-branch depth sets the depth scaling rule,
  because a branch with two transformations has a second-order update term a
  branch with one does not. `Active`. Explains `SOTA-144`, `SOTA-tmpnjlal`
  and `SOTA-tmpr10ct`.
- `SOTA-tmpnjlal` — pick the depth parameterization from the residual branch;
  ...
  `SOTA-144`.
- `SOTA-tmpr10ct` — for a normalized or preconditioned optimizer, width-depth
...
- **The record recommends Muon and matrix preconditioning and never said how
  to muP either.** `SOTA-tmpr10ct` is the answer, and it is short: a
...
- **A confound the authors flag themselves.** For Muon-Kimi-AdamW, standard
  ...
  `SOTA-tmpnjlal`'s conditions and `THEORY-tmpmnsb5`, because it means the
