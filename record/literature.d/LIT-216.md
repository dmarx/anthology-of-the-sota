...
- version: 3
  date: '2026-09-23'
  note: >-
    `compared_against: LIT-tmp0s7m8` declared. This paper ran the comparison
    against MAE that `SOTA-250` quotes — a ViT-H/14 for less compute than MAE
    needed — and the record held the winner without the loser until unit E of
    `#304`. `representation-and-encoding` added so the relation binds, and
    because it is true of the document: the whole claim is about what the
    representation is trained to be.
tags:
- vision-and-graphics
...
# LIT-216: Self-Supervised Learning from Images with a Joint-Embedding Predictive Architecture
...
because [LIT-215](LIT-215.md) is here, and a line whose origin is missing is a line
...
**Audited by [#121](https://github.com/dmarx/anthology-of-the-sota/issues/121); unsourced and unread.** No reading note. See
[LIT-215](LIT-215.md): this is one of a three-paper joint-embedding-predictive cluster
in the same state.
