...
- version: 3
  date: '2026-09-24'
  note: >-
    Corrected against full readings of Self Forcing (NOTE-tmp3okbi) and
    CausVid (NOTE-tmpj1xug). Table 2 is two matched pairs, not one setup.
    The many-step rows were fine-tuned on 70K videos generated by the 14B
...
generated by Wan2.1-14B. The distilled rows are 4-step DMD students. Each
comparison below is within a pair. It bears on the two halves differently.
...
- **Rollout half, noised history: not tested.** The paper describes the
  inference setting for diffusion-forced models as one "where context frames
  are clean and the current frame is noisy" (§1), and labels
  autoregressive inference "Clean KV" (Fig. 2c). The distilled
  diffusion-forcing row replicates CausVid, which caches clean outputs. The
  text doesn't say how the many-step rows were sampled.

CausVid ([LIT-631](../literature.d/LIT-631.md)) adopts per-chunk noise for its student and its causal
teacher, and also rolls out on clean context. So it adopts the training
half only.
