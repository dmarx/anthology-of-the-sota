...
<!-- inactive-ok-file: SOTA-419 SOTA-417 — Proposed, and named as the two practices
...
# LIT-681: Cramming: Training a Language Model on a Single GPU in One Day
...
- **It sources two practices.** `SOTA-419`: under a fixed budget,
  change the architecture only where it cuts step time at constant parameter
  count. `SOTA-417`: filter pretraining text by its tokenizer compression
  ratio.
