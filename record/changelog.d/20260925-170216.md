...
- **DPM-Solver++** — `LIT-tmp6mla4`. Lu et al. (2022), `2211.01095`. Re-measured
...
- **`THEORY-tmp8i6gq`**, `Proposed` — a large guidance scale amplifies the
...
- **`SOTA-tmpylyb1`**, `Active`/`converged` — for guided sampling, use a
...
- **`SOTA-202` v2 gains the precondition it never stated**, with recommendation,
  ...
  images, and `LIT-tmp6mla4` found it because it broke an ODE solve. That
