...
- **`LIT-tmpcv53l`** — *Attention is not all you need: pure attention loses
  rank doubly exponentially with depth* (Dong, Cordonnier and Loukas, 2021;
  ICML 2021), read as `NOTE-tmpsokgi`.
- **`LIT-tmp326fh`** — *Linformer: Self-Attention with Linear Complexity*
  (Wang, Li, Khabsa, Fang and Ma, 2020), read as `NOTE-tmpqs6fj`.
- **`THEORY-tmp91yaa`** — pure self-attention collapses every token onto one,
...
- **"Rank collapse" names two different phenomena in this record, and the
  ...
  *token representations converging to rank 1 through depth in one forward
  ...
  `NOTE-tmpsokgi` carries the table separating them.
