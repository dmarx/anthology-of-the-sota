...
- version: 2
  date: '2026-09-26'
  note: >-
    The family this paper hands off is held one unit later. Nie, Zhang and Patel
    is LIT-tmpbspyz and the account is THEORY-tmpv7nad: guided backprop recovers
    the input in a random CNN. Also records that this paper's line about pooling
    switches and the new one's finding that switches rescue DeconvNet are not in
    conflict — different families, checked rather than assumed.
tags:
...
summary: >-
  ...
  non-linear and which it defers to Nie et al. (2018), now LIT-tmpbspyz.
---
...
- **And a story the paper rules out by name.** "Other than argued in (Gu et al.
  ...
  Pooling switches." Read against [LIT-tmpbspyz](LIT-tmpbspyz.md), which finds
  max-pooling switches are exactly what turn DeconvNet's output from noise into a
  recognisable image, that looks like a clash and is not: this sentence rejects
  Gu et al.'s account of class-insensitivity in the `z⁺` family, and the other
  paper's switches produce *recovery*, which is why DeconvNet is insensitive
  rather than a rival cause of it. Checked rather than assumed.
...
Perplexing Behaviors of Backpropagation-based Visualizations*, held one unit
later as [LIT-tmpbspyz](LIT-tmpbspyz.md) (its identifier was resolved against arXiv rather than
recalled, which is why the handoff was followable). Both documents are corrected
...
| Guided Backprop, Deconv, RectGrad | partial image recovery | [THEORY-tmpv7nad](../theory.d/THEORY-tmpv7nad.md) |
