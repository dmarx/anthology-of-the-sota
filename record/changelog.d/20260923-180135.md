...
- **SwAV** — `LIT-tmp6nq8y` and `SOTA-tmpoe0zb`. Unit D of `#304`. Predict
  ...
  **`SOTA-tmpoe0zb` files multi-crop**, which is the separable half: two
...
- **DINOv2** — `LIT-tmpfwfu3` and `SOTA-tmpj9zpp`. The thesis is about data,
  ...
  **`SOTA-tmpj9zpp` files curation by retrieval** — use the curated datasets
...
- **`THEORY-tmpf89jm` v2 — a fifth account, and a group declining to
