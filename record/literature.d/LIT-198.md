...
# LIT-198: Why Low-Precision Transformer Training Fails: An Analysis on Flash Attention
...
**The practice is [SOTA-tmpbp6p9](../practices.d/SOTA-tmpbp6p9.md), `Active`**, extending [SOTA-085](../practices.d/SOTA-085.md). That stub
says to use flash attention wherever the hardware supports it and says
nothing about precision, so the configuration that fails is not something it
recommends so much as something it fails to rule out — which is the shape
[ADR-012](../decisions.d/ADR-012.md) is about.
