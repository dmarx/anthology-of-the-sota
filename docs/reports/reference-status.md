...
**0 documents cited without acknowledgement.** Not listed: 14 citations someone has already vouched for with an `inactive-ok:` comment at the citing site.
...
**0 codes unaccounted for.** Not listed: 5 mentions marked deliberate with an `unresolved-ok:` comment (same syntax and scopes as `inactive-ok:` above).
