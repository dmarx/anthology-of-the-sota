...
- QLoRA (`LIT-tmpu3hfw`, read in `NOTE-tmpdlk6r`) and the two practices it
  supports, which are separable claims that happen to share a paper:
  `SOTA-tmpasvp1` — store the frozen base in 4-bit NormalFloat and dequantize
  ...
  `SOTA-tmpdf5n5` — put an adapter on every linear layer rather than the
