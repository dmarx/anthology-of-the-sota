...
**No practice of its own: it is the origin of a unit the record already
recommends a variant of.** [SOTA-034](../practices.d/SOTA-034.md) recommends SwiGLU and now carries this
paper's argument in its "Where the gate came from" section — the gate is
there so depth works, and the *linear* branch is the point. Filing the
ancestor separately would offer a reader a choice between a design and its
own improved descendant.

The 2016 argument has also become load-bearing in a way it was not: the
unbounded linear branch is precisely what the two low-precision objections
now target, which is why both replacements soft-cap it rather than removing
it.
