...
# SOTA-tmpe3b5u: Switch from minibatch SGD to local SGD at the first learning-rate decay, rather than choosing between them
...
[LIT-tmpx82es](../literature.d/LIT-tmpx82es.md) gives `H = O(sqrt(T/(Kb)))` for `T` total steps, `K` workers and
...
The argument in [LIT-tmpxaxzy](../literature.d/LIT-tmpxaxzy.md) is that large-batch SGD loses generalization
...
exactly what [SOTA-tmpntee6](SOTA-tmpntee6.md) says to be careful about: a large-batch run whose
