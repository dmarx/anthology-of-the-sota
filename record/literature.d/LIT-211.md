...
version: 1
tags:
# Retagged from the report of unbound lineage. The paper is about LLM
# fine-tuning -- it says so in its title -- and `adaptation-and-tuning` names
# fine-tuning in its blurb. SOTA-154, the practice it introduced, and five of
# its six descendants already carry this topic; this note was the odd one.
- adaptation-and-tuning
# inactive-ok: ADR-tmpoxkk7 — Proposed, and what permits this tag
# Secondary, restored: it substitutes one optimizer for another, which is a training-optimization claim as well as a fine-tuning one (ADR-tmpoxkk7).
