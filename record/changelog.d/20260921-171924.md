...
- **`LIT-tmpzcf35`** — Denoising as Projection (Zhang et al., 2026), read as
  `NOTE-tmpyuelo`. Standard gradient guidance adds the objective gradient
...
- **`SOTA-tmprcn0m`** — apply the objective gradient before the denoiser, not
  after it, when guiding diffusion toward a task objective. `Proposed`.
- **`THEORY-tmphpdiq`** — the Stein posterior-mean denoiser acts as an
...
- **`THEORY-tmphpdiq` is `Proposed` despite having proofs.** The theorems
