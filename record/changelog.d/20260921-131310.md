...
- `LIT-tmpglf4i` / `NOTE-tmpsi60o` — Kojima et al., *Large Language Models are
  Zero-Shot Reasoners* (`ARXIV-2205.11916`). Read. `extends: LIT-467`.
- `LIT-tmp43eio` / `NOTE-tmpkw9rw` — Wang et al., *Self-Consistency Improves
  Chain of Thought Reasoning* (`ARXIV-2203.11171`). Read. `extends: LIT-467`,
  tagged `inference-optimization` as well.
- `SOTA-tmpi7boj` — try the single step-by-step instruction before writing
  exemplars. `Active`, `consensus: universal`. Extends `SOTA-279`.
- `SOTA-tmp9d9au` — sample several reasoning paths and take the majority
