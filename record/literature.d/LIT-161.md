...
- version: 2
  date: '2026-09-18'
  note: >-
    Adds `attention-techniques`. That topic's blurb covers "alternative mechanisms", which is what a selective state-space model is, and LIT-162 — in this same line — establishes that SSMs and attention are one object seen from two ends. It is the value that names the line: the six-member component held nothing in common until it was written down (ADR-tmphn2vp).
tags:
- model-architecture
...
# LIT-161: Mamba: Linear-Time Sequence Modeling with Selective State Spaces
...
**No practice of its own: the recommendation it contains has a successor the
record already carries.** "Make the state-space parameters input-dependent"
is the claim, and every module the record recommends is a descendant of it —
the gated delta rule and its channel-wise variant both inherit selectivity
and add to it. Filing it separately would give a reader two recommendations
where the later one supersedes the earlier, which is what a `Sequence`
section is for instead.
