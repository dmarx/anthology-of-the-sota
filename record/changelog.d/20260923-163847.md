...
- **The collapse-avoidance cluster** — four notes (`LIT-tmpawe8p` BYOL,
  `LIT-tmp3roys` SimSiam, `LIT-tmpovcux` Barlow Twins, `LIT-tmpipfcy`
  ...
  **`SOTA-tmp1kmsu` — break the symmetry.** Drop negatives entirely and match
  ...
  **`SOTA-tmpacu6q` — or put the constraint in the loss.** A hinge keeping
  ...
  **`SOTA-tmp3wrpo` — the three-line collapse detector.** Log the per-channel
  ...
  **`THEORY-tmpf89jm` (`Proposed`) — nobody knows why any of this works.**
