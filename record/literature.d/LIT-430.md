...
# LIT-430: RWKV: Reinventing RNNs for the Transformer Era
...
- **`R`, `W`, `K`, `V`** — receptance, weight (the decay), key, value. The
  `W` is `LIT-431`'s learned position bias restricted to a **decaying
...
- **Train in parallel, run as an RNN.** The same layer has a
  ...
  organised around and that `LIT-428` first established was possible
