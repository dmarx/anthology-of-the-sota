...
- version: 3
  date: '2026-09-22'
  note: >-
    The decision stands and bought more than it claimed. `luria concretize`
    rewrites every file in `code.globs`, so putting `luria.yaml` in that list
    did not only turn on the lint — it also made temporary codes written into
    the config concretize on push, which this document never said and nobody
    had checked. Measured on 2026-09-22 with a scratch decision: a temp code
    in `luria.yaml`, plain and backticked, both became the numbered code.
    Recorded because the record had drifted to the opposite belief and acted
    on it.
tags:
- record
- ci
...
<!-- unresolved-ok-file: ADR-tmpxxxxx — not a citation: a stand-in temporary code, used twice to show the shape `luria link --fix` leaves alone. -->
...
**What this change bought and did not say (v3).** `luria concretize`
rewrites every occurrence of a renamed code in `doc_files()` *and in every
file matching `code.globs`*. Adding `luria.yaml` to that list therefore did two
things, and this decision only claimed one: the lint reads the config, **and
concretization now repairs it**. Measured on 2026-09-22 — a scratch decision's
temporary code, written into `luria.yaml` both bare and inside backticks, came
back as the numbered code on both counts.

That matters because the record subsequently drifted to the opposite belief.
A comment written into this very file on 2026-09-22 asserted that a temporary
code here "would go stale with no mechanism to fix it", and cited this
document for it. The Context above is accurate history — nothing *was*
rewriting the config, before this decision. The Consequences never recorded
that the decision itself ended that, so a later reader (this one) took the
Context for the current state.

**And the backtick theory is half wrong.** The record has been repeating,
since 2026-09-21, that concretization "does not reach inside backticks". It
does: `_rewrite_files` is a plain string replace over whole file contents.
What cannot reach inside backticks is `luria link --fix`, which upgrades a
*bare* already-concretized code and leaves both `` `ADR-tmpxxxxx` `` and a
hand-written `[ADR-tmpxxxxx](ADR-tmpxxxxx.md)` untouched — also measured. The
two commands were conflated, and the citations that needed hand-fixing on
2026-09-21 and 2026-09-22 were cross-branch codes with no pending rename for
concretization to apply ([#211](https://github.com/dmarx/anthology-of-the-sota/issues/211)), which is a different defect from the one
the record named.
