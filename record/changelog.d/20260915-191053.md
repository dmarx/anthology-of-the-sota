...
- **Four papers closing the evolution-strategies line**, each with a `Read`
  reading note:
  - `LIT-tmppbfp5` / `NOTE-tmpl3gt3` — *Evolutionary Strategies lead to
  ...
  - `LIT-tmp4w505` / `NOTE-tmpmfvn0` — *Matching Accuracy, Different Geometry*
    (Hoy et al., 2026). The mechanism the line was missing.
  - `LIT-tmprde8b` / `NOTE-tmpwuo1b` — *ESSAM* (Sun et al., 2026). `extends:`
    `LIT-211`. Sharpness-aware minimization transposed into zeroth order.
  - `LIT-tmpqknia` / `NOTE-tmpuefjy` — *ESSA* (Korotyshova et al., 2025). The
    earliest paper in the line and the last one filed.

- **`THEORY-tmpt76ks` — an ES update is mostly a loss-invariant random walk
...
- **`SOTA-tmpdcmgg` — stop ES post-training when the target task converges,
  and buy accuracy with population size rather than more steps.** `Proposed`,
  `emerging`. Countdown peaks at about 200 iterations; `LIT-tmppbfp5` ran to
  ...
  is available — and `LIT-tmp4w505`'s continual result is explicitly
  conditioned on "when its iteration budget is controlled".

### Changed

- **`SOTA-154` gains a second `contested_by:` and a scale boundary.**
  `LIT-tmppbfp5` is the independent replication attempt that did not reproduce
  the ordering — GRPO ahead on three of four settings, at 1B and 1.5B. It also
  gains `LIT-tmp4w505` as a source: a fourth independent group, at 4B, both
...
- **The record held a reply before it held the claim.** `LIT-230`'s entire RQ2
  is an answer to `LIT-tmppbfp5`, and the record filed the answer first,
  because the citation graph was followed forward from a 2026 paper and never
  backward. `LIT-tmpqknia` is the same error at the other end of the line: the
...
- **Three papers were arguing about one quantity without knowing it.**
  `LIT-tmppbfp5` read ES's drift as the cause of forgetting, `LIT-230` read it
  as functionally sparse and harmless, `LIT-231` read it as proof the search
  finds nothing. `THEORY-tmpt76ks` says all three are describing a
