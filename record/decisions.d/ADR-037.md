...
- version: 2
  date: '2026-09-23'
  note: >-
    Promoted to Active. Part 2 reads as out of date: luria 0.28.2 fixed
    LU-#272 (LU-ADR-106), and ADR-055 declared `invariant: tags` on
    `SOTA.source` once the backlog reached zero. The decision stands, and
    the 'cannot be declared here yet' text records the state on the day it
    was written.
tags:
- mechanism
date: '2026-09-16'
...
# ADR-037: Fail the build on `source-mismatch`, and take the source invariant upstream
