...
- ADR-049
status_note: >-
  by ADR-049, which reversed this document's premise that an unbound edge is
  not a defect, and took the seams it described to zero. Its options were
  each taken elsewhere: more than one topic per document by ADR-035, a
  filing rule by ADR-026 and ADR-046, and new topics by ADR-050.
formerly:
- ADR-tmpowu4t
...
# ADR-024: The topic vocabulary has four recurring seams — options, not a decision
