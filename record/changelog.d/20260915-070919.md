...
- **A third scheme, `THEORY`, for claims about why something works.** A
  ...
  had nowhere to put the second. [ADR-tmpvv7zp](record/decisions.d/ADR-tmpvv7zp.md) is the decision.
...
- **Four documents, filed with the scheme.** The batch-normalization pair is
  the case it was shaped around: `THEORY-tmp69thf`, the internal-covariate-shift
  account the technique was published with, is `Rejected`, and
  `THEORY-tmpn1kz1` corrects it — while the practices remain `Active` and
  ...
  (`THEORY-tmp6auqn`, corrected by `THEORY-tmpqvo94`) is the other half of
