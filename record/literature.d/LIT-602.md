...
- version: 2
  date: '2026-09-23'
  note: >-
    Its headline comparison with word2vec is corrected by LIT-tmp56jtw,
    which re-ran it with both methods tuned; the standing section says so.
tags:
...
about 10 samples. The margin is 2.6 points at 6B tokens.

**The re-tuned comparison reverses it.** [LIT-tmp56jtw](LIT-tmp56jtw.md) (Levy, Goldberg and
Dagan 2015) trains both methods on one corpus, allows w + c for both and tunes
both. SGNS beats GloVe on every task, including at 10.5B tokens. It is filed
as `corrects` this paper on that claim. The formulation and the ratio
argument are not in question.
