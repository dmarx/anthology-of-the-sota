...
- version: 2
  date: '2026-09-23'
  note: >-
    Heading corrected from a template placeholder to the document's code.
    The decision is unchanged and stays Proposed.
- version: 3
  date: '2026-09-23'
  note: >-
    The decision stands, and one of its rejected alternatives does not.
    v1 and v2 rejected adding a topic for this kind of claim, on the
    argument that a category added to admit a single document is how a
    scope stops being one. ADR-059 has since made the topics an axis rather
    than a scope, so a document the axis cannot place is a finding about
    the axis. This decision's own cost paragraph had recorded that finding:
    the tag it used was the closest available, not a good fit. v3 takes the
    alternative and adds `signal-structure`. It retags THEORY-022 and
    LIT-410, and appends the topic to THEORY-080.
tags:
- record
date: '2026-09-17'
summary: >-
  ...
  exception framing, and sourcing a practice from it. Adding a topic, which
  v1 rejected, is taken in v3 under [ADR-059](ADR-059.md): `signal-structure`.
---

<!-- inactive-ok-file: THEORY-080 — Proposed, and named in the history as one of the documents this revision retags, not cited as settled -->

# ADR-045: A claim about the signal is admissible however it was measured, including on people

## Context

[LIT-410](../literature.d/LIT-410.md) is a paper in *Language* about
...
**Such a claim takes `signal-structure` as its topic (v3).** The word is
for what the data itself is like, independent of any model: the statistics
and structure of language, images and other signals, which methods exploit
or have to respect. That covers frequency spectra, heavy-tailed and Zipfian
distributions, and what counts as a unit. v1 filed [THEORY-022](../theory.d/THEORY-022.md) under
`representation-and-encoding` and said in its own cost paragraph that this
was the closest word available, not a good fit. Under [ADR-059](ADR-059.md) that sentence
was the finding, and the topic is the answer to it.
...
- **Add a topic for it.** Rejected in v1 and v2, and taken in v3. The
  argument was [DP-008](../../docs/design-principles.md#dp-8)'s corollary: a category added to admit a single
  document is how a scope stops being one. [ADR-059](ADR-059.md) answered that the topics
  are not a scope, so there was no scope to stop being one. The word chosen
  is broader than the v1 candidate, `linguistics`. It names the kind of claim
  (a property of the signal), not the field that measured it, which is this
  decision's own rule applied to the vocabulary.
...
**What it costs.** In v1, `representation-and-encoding` held a theory about
the structure being encoded rather than about encoding it for a network, with
the tag recorded as the closest available, not a good fit. v3 fixes that
instead of recording it (see the history). The theory keeps
`representation-and-encoding` as a second topic, because the practices under
that word act on the structure this claim describes.
