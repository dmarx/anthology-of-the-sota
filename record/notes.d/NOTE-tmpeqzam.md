...
| C2 | SwiGLU and GeGLU improve pre-training, fine-tuning and supervised translation at no speed cost | moderate | Table 1 and Table 2; effect well outside seed std. Not independent of LIT-030, which ran the same codebase and configuration |
...
Reimplements and scores work the record holds: GLU Variants (LIT-030) and its
GLU origin (LIT-199, as "GLU"), RMSNorm (LIT-023), ReZero (LIT-047), the
sparsely-gated MoE (LIT-188), Switch (LIT-189), ALBERT's sharing and
factorized embeddings (LIT-668), and lightweight/dynamic convolution
(LIT-020). Its baseline is the T5 recipe (LIT-425). The Universal Transformer,
...
- **SOTA-034 (SwiGLU).** The gain reproduces with five seeds and on all four
  task families. The re-run is in LIT-030's own codebase and configuration, so
  it corroborates the measurement, not its transfer. The practice now says so.
- **SOTA-182 (RMSNorm).** Loss reproduces. The speedup is about 2–5% here,
  against 7–64% in LIT-023, and downstream wins do not survive the switch to
  learned positions. The practice now says so.
- **SOTA-150 (MoE).** The quality gain at matched FLOPs reproduces, in the
  codebase MoE and Switch were built in, at about 9% lower step rate. Added.
- **SOTA-051 (ReZero).** ReZero *as a replacement for LayerNorm* fails
  badly here, which bounds LIT-047's claim that the normalization and warmup
...
- **SOTA-403 (share attention, not FFN).** The split is not tested. What is
...
- **SOTA-190 (depth first).** The depth sweep is non-monotone and slower with
  ...
  as a bound.
- **SOTA-032 (pre-norm).** Not evidence. Pre-norm is the untested baseline,
