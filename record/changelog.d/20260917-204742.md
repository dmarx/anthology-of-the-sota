...
- `THEORY-tmpvyj77` (`Proposed`) — the four stages, of which detokenization is
  ...
  falsify.
- `LIT-tmp8gq9j` and `LIT-tmp2gcne` — the inner-lexicon line, which the two
...
- `THEORY-tmpkh59b` (`Active`) — a model builds a latent vocabulary in its
...
- `SOTA-tmp5b7vv` (`Proposed`, `unreplicated`) — add a multi-token word to a
...
- `THEORY-tmp1rmwn` (`Active`) — a softmax head with nothing to attend to must
...
- `LIT-tmpfe659` — Bondarenko et al. (2023), the quantization-side diagnosis
...
- `LIT-tmpeajzz` — Miller (2023), "Attention Is Off By One". The version most
  people have read, filed for provenance and reach rather than evidence: it
  reports no experiments and says so.
- `SOTA-tmp8ldmf` (`Proposed`, `unreplicated`) — stretch and clip the softmax
