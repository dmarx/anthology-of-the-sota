...
- `introduced_by` — optional, one or more `LIT` codes when present (luria.yaml: schemes.SOTA.references.introduced_by) — *the work that first made this recommendation, which is frequently not the work that evidenced it. Separating origin from evidence is ADR-029, refined by ADR-030; with one field the record would read as though every practice began with the paper it cites. Leave it empty only to say the record looked and found no origin document, with a comment saying so — ADR-053.*
...
- `tags` — required, one or more of `training-optimization`, `systems-optimization`, `numerics-and-precision`, `model-stability`, `distributed-optimization`, `data-pipeline`, `attention-techniques`, `model-architecture`, `inference-optimization`, `adaptation-and-tuning`, `in-context-learning`, `representation-and-encoding`, `analysis-and-evaluation`, `generative-modeling`, `vision-and-graphics`, `multimodal-learning`, `deployment-and-society`, `biomolecular-modeling`, `signal-structure`, `concept-geometry`, `tiny-models`, `capability-thresholds` (luria.yaml: schemes.SOTA.fields.tags; vocabulary 'topics': values) — *The twenty-two kinds of claim this anthology files against — the primary axis of both the practice registry and the reading list, and the same twenty-two for each (ADR-026). The filing rule is the point: take a domain topic when the claim is ABOUT the domain; a claim merely discovered in one still takes its kind. The axis organizes the record, it does not bound it: content the list cannot place is evidence the list is short (ADR-059).*
...
- `tags` — required, one or more of `training-optimization`, `systems-optimization`, `numerics-and-precision`, `model-stability`, `distributed-optimization`, `data-pipeline`, `attention-techniques`, `model-architecture`, `inference-optimization`, `adaptation-and-tuning`, `in-context-learning`, `representation-and-encoding`, `analysis-and-evaluation`, `generative-modeling`, `vision-and-graphics`, `multimodal-learning`, `deployment-and-society`, `biomolecular-modeling`, `signal-structure`, `concept-geometry`, `tiny-models`, `capability-thresholds` (luria.yaml: schemes.THEORY.fields.tags; vocabulary 'topics': values) — *The twenty-two kinds of claim this anthology files against — the primary axis of both the practice registry and the reading list, and the same twenty-two for each (ADR-026). The filing rule is the point: take a domain topic when the claim is ABOUT the domain; a claim merely discovered in one still takes its kind. The axis organizes the record, it does not bound it: content the list cannot place is evidence the list is short (ADR-059).*
...
- `tags` — required, one or more of `training-optimization`, `systems-optimization`, `numerics-and-precision`, `model-stability`, `distributed-optimization`, `data-pipeline`, `attention-techniques`, `model-architecture`, `inference-optimization`, `adaptation-and-tuning`, `in-context-learning`, `representation-and-encoding`, `analysis-and-evaluation`, `generative-modeling`, `vision-and-graphics`, `multimodal-learning`, `deployment-and-society`, `biomolecular-modeling`, `signal-structure`, `concept-geometry`, `tiny-models`, `capability-thresholds` (luria.yaml: schemes.LIT.fields.tags; vocabulary 'topics': values) — *The twenty-two kinds of claim this anthology files against — the primary axis of both the practice registry and the reading list, and the same twenty-two for each (ADR-026). The filing rule is the point: take a domain topic when the claim is ABOUT the domain; a claim merely discovered in one still takes its kind. The axis organizes the record, it does not bound it: content the list cannot place is evidence the list is short (ADR-059).*
