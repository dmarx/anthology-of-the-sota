...
- **`LIT-tmpqux2k`** — *Leaner Transformers: More Heads, Less Depth*
  (Saratchandran, Teney and Lucey, 2025), read as `NOTE-tmph5061`.
- **`LIT-tmpc05l0`** — *Transformers Learn Low Sensitivity Functions*
  (Vasudeva et al., 2024), read as `NOTE-tmpwuv3u`.
- **`LIT-tmp81ryb`** — *Ghost in the Transformer* (Wang et al., 2025), read as
  `NOTE-tmp42huo`.
- **`LIT-tmpdi9f9`** — *Concepts Whisper* (Acharya, Rimal and Dhakal, 2026),
  read as `NOTE-tmpruk19`.
- **`SOTA-tmpwlajs`** — trade attention heads for depth. `Proposed`,
  `unreplicated`.
- **`SOTA-tmp3ot5z`** — verify a model's lineage from the singular spectra of
  its attention products. `Proposed`, `unassessed`.
- **`THEORY-tmpn1efa`** — transformers are biased toward low-sensitivity
  functions, and that bias is what their robustness is made of. `Proposed`.
- **`THEORY-tmphj2w6`** — contextual concept directions sit in the
...
- **The provenance practice is `Proposed` for a specific missing
  ...
  evidence and proof.
- **A paper that grades its own claims.** `LIT-tmpdi9f9` reports concept
