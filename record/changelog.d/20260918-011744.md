...
- `LIT-tmppydca` — **Qwen2.5 Technical Report**. Named in forty-five documents
...
- `LIT-tmpnraor` — **Chollet, On the Measure of Intelligence**, which
  introduces ARC-AGI.
- `LIT-tmpwqnta` — **Clark et al., the AI2 Reasoning Challenge**, which is a
  different benchmark with the same acronym.
- `LIT-tmpn6zv1` — **HellaSwag**, whose distractors are adversarially filtered
  against 2019's models, so its difficulty is relative to them.
- `LIT-tmpx3u6l` — **MBPP**, the other half of the code-generation pair whose
