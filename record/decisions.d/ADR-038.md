...
- version: 2
  date: '2026-09-23'
  note: >-
    Promoted to Active for its decision that a note derives its topics from
    its paper. One clause no longer holds: 'no cross-scheme relation in this
    record asserts an invariant' was reversed by ADR-055, which declared
    `invariant: tags` on `SOTA.source` after taking its backlog to zero.
tags:
- record
- taxonomy
date: '2026-09-16'
...
# ADR-038: A note has no topics of its own: it derives them from its paper

<!-- inactive-ok-file: ADR-036 — Superseded, and named as reasoning
     this one continues: it read the same-scheme findings through and named the
     four categories. -->
...
- **Assert the agreement with `invariant: tags` on `NOTE.paper`.** What this
  ...
  source — which is [LU-ADR-089](https://github.com/dmarx/luria/blob/main/record/decisions.d/ADR-089.md)'s argument upstream and the reason `published:`
