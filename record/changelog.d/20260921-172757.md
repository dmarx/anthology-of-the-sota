...
- **`LIT-tmpksq2o`** — Noise Hypernetworks (Eyring et al., 2025), read as
  `NOTE-tmptc2jq`. Train a LoRA hypernetwork once to predict an improved
  initial noise for a frozen step-distilled generator, instead of running
  reward-guided noise optimization per sample.
- **`SOTA-tmplndwz`** — steer a distilled generator by modulating its input
  noise, not by fine-tuning its weights. `Proposed`.
- **`THEORY-tmpq2xg2`** — the regularizer that is intractable in data space is
...
- **`THEORY-tmpq2xg2` is `Proposed` because an upper bound is not a
