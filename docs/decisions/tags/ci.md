...
| [ADR-054](../../../record/decisions.d/ADR-054.md) v2 | The four commands are five, and two of them are invariants a hook can hold | `luria repair` is in the documented sequence, which had four commands and needed five — and, per v2, `luria link --fix` stays, because neither command contains the other. A `Makefile` wraps the sequence and a tracked `pre-commit` hook refuses generated views on a branch, which is [ADR-018](../../../record/decisions.d/ADR-018.md) stated as an invariant rather than remembered. Rejected: gitignoring `docs/` on branches, since main tracks them; and leaving the sequence undocumented on the grounds that `luria --help` is authoritative, which is true and is not where anybody looks. | Proposed |
